...
<div class="container">
	...
	$sql_carrinho = "SELECT id_produto,qtd_produto FROM carrinho WHERE id_usuario='$id_usuario'";
	$re = mysqli_query($conexao, $sql_carrinho);
	$num_produtos_carrinho = mysqli_num_rows($re);
	$total_compra = 0;
	$x = 0;
	unset($sql,$result,$linha);
	while($lc = mysqli_fetch_array($re)){
	if (($x % 2)==0) {
	$bgcolor="#FFFFFF";
	}else {
	$bgcolor="#DADADA";
	...
	<tr>
	<td colspan="6"><form name="formas" action="compra_finalizada.php?idpro=<?php echo $id_produto?>&qtd=<?php echo $qtd?>&idcli=<?php echo $id_usuario?>&total=<?php echo $total_compra?>" method="post"></td>
	</tr>
	<tr>
	...
	<tr>
	<td align="center" colspan="6"><button type="submit" class="btn btn-success"> Finalizar a compra</button></form></td>
	</tr>
	</table>
</div>
</body>
